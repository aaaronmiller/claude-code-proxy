...
def install_dependencies_pip(project_root: Path, venv_path: Path) -> bool:
    ...
    try:
        run_command([str(pip_exe), 'install', '-r', str(project_root / 'requirements.txt')], cwd=project_root)
        print_success("Dependencies installed")
        return True
    except Exception as e:
        print_error(f"Failed to install dependencies: {e}")
...
Examples:
  ...
  python quickstart.py --no-launch    # Setup without launching proxy
    ...
    parser.add_argument(
        '--no-launch',
        action='store_true',
        help='Setup without launching the proxy'
    )
...
if __name__ == '__main__':
    try:
        ...
        print("If the problem persists, visit: https://github.com/holegots/claude-code-proxy/issues")
        sys.exit(1)
