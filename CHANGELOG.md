...
### Changed
- **`xx` launcher: new grammar `<agent><run>[route][model]` + single-screen help** (`scripts/xx`, `scripts/install-aliases.sh`) — run is now case-encoded (lowercase = interactive, UPPERCASE = headless: `i/I c/C r/R`); route defaults to proxy; model selectors (`m`best `f`free `t`tool `r`reason `v`verify `s`fast `0-9`profile) are **only valid on the proxy route** (rejected with bypass/direct). Headless runs require a prompt (text, `-p`, or `--stdin`). Added `doctor`/`models`/`sessions` subcommands and `-P/--provider`, `-n/--dry-run`, `-v/--verbose` flags. Help is multi-column and fits one terminal screen. Direct route now also strips the RTK wrapper (truly raw). Fixed a `set -e` silent-exit bug (model helper returned non-zero on interactive runs). Installer now **symlinks** `~/.local/bin/xx` → `scripts/xx` so the repo source is canonical and edits are live without reinstall. Legacy aliases (`xx pip`/`pcp`) still parse.

### Added
- **Usage tracker: cache-token + transformation visibility** (`src/services/usage/usage_tracker.py`, `src/api/endpoints.py`) — `api_requests` gains `cached_tokens` (prompt-cache reads, cost-weighted ~0.1×), `transformed` (1 when a protocol conversion occurred), and `transform_type` (e.g. `claude->openai/<provider>`). Columns added via idempotent `ALTER` migrations; values persisted by an additive post-INSERT `UPDATE` so the fragile 30-col main INSERT is untouched and pre-existing DBs degrade gracefully. All four `log_request` call sites in the Claude `/v1/messages` path now pass these. Combined with the existing `original_model`→`routed_model`/`resolved_model`, `provider`, and `duration_ms` columns, every request is now classified by **requested-vs-actual model, actual provider, latency, full token breakdown (incl. cache), and whether a transformation occurred**. Recording requires `TRACK_USAGE=true` (already set in `.env`); it only populates when traffic actually flows through the proxy (direct claude.ai/Pro traffic bypasses it).

### Changed
- **Restored Anthropic upstream redirect (lost when headroom dropped `--anthropic-api-url`)** (`scripts/headroom-start.sh` + local patch to installed `headroom/proxy/server.py`) — headroom 0.5.7's `proxy` CLI has no anthropic-upstream override (`ANTHROPIC_API_URL` is hardcoded to `api.anthropic.com`; only OpenAI/Gemini are overridable). Patched the installed package so `ANTHROPIC_API_URL` reads `os.environ["ANTHROPIC_TARGET_API_URL"]` (mirroring headroom's own `OPENAI_TARGET_API_URL` convention), and `headroom-start.sh` now exports `ANTHROPIC_TARGET_API_URL="${HEADROOM_ANTHROPIC_UPSTREAM-$HEADROOM_UPSTREAM_URL}"` so anthropic traffic chains through the upstream by default (set `HEADROOM_ANTHROPIC_UPSTREAM=""` to go direct). NOTE: the server.py change lives in site-packages and will be wiped by `pip install -U headroom` — reapply after any headroom upgrade.

### Fixed
- **SPA deep-link routing + port-agnostic web UI** (`src/main.py`, `web-ui/src/lib/services/config-client.ts`) — The `/` `StaticFiles` mount swallowed every path, so client routes (`/settings`, `/assignments`, …) 404'd on hard-load/refresh and the explicit `/config`/`/settings` routes were dead. Replaced with a file-or-`index.html` fallback (guards `/api` + `/v1` so they still 404 correctly); legacy settings page moved to `/settings-legacy`. The web-ui client now uses same-origin URLs in the built bundle (was hardcoded to `:8082`, breaking the UI on any other port); `vite dev` still targets `:8082`. Live-verified in-browser: `/settings` renders all 14 groups and round-trips load + edit + save through `/api/config/manifest`.
- **Wired the two unfed Prometheus metrics** (`src/core/circuit_breaker.py`, `src/api/websocket_logs.py`) — `proxy_circuit_breaker_state` and `proxy_cascade_switches_total` were defined and graphed in Grafana (`deploy/observability/grafana/dashboards/clutch-gateway.json`) but never incremented, leaving the "Cascade Switches" and "Active Profiles"-adjacent panels empty. Circuit-breaker transitions now route through a single `_set_state` point that mirrors state to the gauge; `log_cascade(action="switch")` (the one choke point for all 17 switch sites) now increments the cascade counter. Both emissions are best-effort and never affect breaker/cascade logic. (`active_profiles` was already refreshed on every `/metrics` scrape.) Tests: `tests/test_metrics_wiring.py`.

### Added
- **Manifest-driven config parity across Web UI + TUI** (`src/core/config_manifest.py`, `src/api/web_ui.py`, `web-ui/src/routes/settings/`, `src/cli/settings_tui.py`) — All 64 `config_manifest` settings are now editable from both the modern Svelte web-ui and a TUI, not just CLI flags + the legacy static page. Adds `GET /api/config/schema` (groups + per-setting render hints from the manifest) and `GET`/`POST /api/config/manifest` (generic, type/choice-validated save that persists to `.env` and hot-applies to `os.environ`; masked secrets are never overwritten). New web-ui `/settings` route renders each setting by its `web_component` (switch/select/number/textarea/input) grouped by section; new `python -m src.cli.settings_tui` does the same over Textual, posting to the same endpoint for cross-surface consistency. Tests: `tests/test_config_schema_api.py`, `tests/test_config_manifest_save.py`, `tests/test_settings_tui.py`.
- **OpenRouter Fusion proxy alias and CLI** (`src/core/fusion.py`, `src/cli/fusion.py`, `docs/guides/fusion.md`) — Adds env-configurable Fusion profiles, routes `fusion`/`ccp/fusion`/`openrouter/fusion` requests to OpenRouter Fusion, and exposes `ccp-fusion <profile> <prompt>` for arbitrary one-shot use from Codex, Hermes, Pi Agent, Qwen, or shell without MCP token overhead.
- **F18 global quota-aware allocator wired into runtime routing** (`src/core/model_scan_runtime.py`, `src/core/quota_runtime.py`, `src/core/proxy_chain.py`, `src/services/probe_runtime.py`, `src/api/endpoints.py`) — Activates the previously dead-code allocator. `ModelScanConfig` gains `allocator_enabled` (default OFF), `session_profiles`, `allocator_slot_map`, `static_quota`, and `quota_nominal_calls`. When enabled with session profiles, `reload_model_scan` runs the satisfice-then-maximize allocator over the routing snapshot and writes per-`(profile, role)` overlays into the **existing** `resolve_profile_binding` request-path consumer (no new request-path code, no registry writes). `quota_runtime.collect_meters` assembles `QuotaMeter`s from live/static provider fractions (best-effort, never raises; empty → pure floor+fitness routing). True `api_model`/`base_url` are recovered from the snapshot (allocator runs in `model_id` space). `GET /api/model-scan/dashboard` now reports the allocation; `POST /api/model-scan/probe` runs the F03/F11 latency/availability probe over the live picks. Default-off and a verified no-op when disabled or unconfigured. Tests: `tests/test_allocator_integration.py`, `tests/test_quota_runtime.py`, `tests/test_probe_runtime.py` (+ existing `tests/test_allocator_dryrun.py`, `tests/test_session_profiles.py`, `tests/test_probe.py`, `tests/test_quota_adapters.py`).
...
### Fixed
- **Fusion configuration surface gaps closed** (`src/static/settings.html`, `src/cli/advanced_config.py`, `docs/feature-parity.md`) — Adds Fusion to the web settings group list, adds a real advanced-TUI Fusion menu, closes the remaining TUI manifest gaps (70/70 env vars referenced), and refreshes stale configuration-count docs.
