...
class RequestLogger:
    ...
    def log_request_start(
        ...
        endpoint_provider = (
            "openrouter"
            if "openrouter" in endpoint_name
            else "openai"
            if "openai" in endpoint_name
            else "anthropic"
            if "anthropic" in endpoint_name
            else endpoint_name
        )
        ...
        if USE_RICH and console:
            ...
            if workspace_name:
                text.append(
                    f" {workspace_name} ", style=f"bold white on {session_color}"
                )
                text.append("  ", style="")
            ...
            if context_limit > 0 and input_tokens > 0:
                ...
                ctx_color = (
                    "green" if ctx_pct < 50 else "yellow" if ctx_pct < 80 else "red"
                )
                ...
                text.append(f"IN:{fmt_tokens(input_tokens)} ", style="cyan")
            ...
            if reasoning_config:
                ...
                if isinstance(reasoning_config, OpenAIReasoningConfig):
                    think_str = (
                        fmt_tokens(reasoning_config.max_tokens)
            ...
            if context_limit > 0 and input_tokens > 0:
                ctx_pct = (input_tokens / context_limit) * 100
                parts.append(
                    f"CTX:{fmt_tokens(input_tokens)}/{fmt_tokens(context_limit)} ({ctx_pct:.0f}%)"
                )
            elif input_tokens > 0:
        ...
        if USE_RICH and console:
            ...
            if usage:
                ...
                if output_limit > 0 and output_tokens > 0:
                    ...
                    out_color = (
                        "green" if out_pct < 50 else "yellow" if out_pct < 80 else "red"
                    )
                ...
                if thinking_tokens > 0:
                    text.append("THINK:", style="dim")
                    text.append(
                        f"{format_tokens(thinking_tokens)} ", style="bold magenta"
                    )
            ...
            if output_tokens > 0 and duration_ms:
                ...
                speed_color = (
                    "green" if tok_s > 50 else "yellow" if tok_s > 20 else "red"
                )
            ...
            if usage:
                if context_limit > 0 and input_tokens > 0:
                    ctx_pct = (input_tokens / context_limit) * 100
                    parts.append(
                        f"CTX:{format_tokens(input_tokens)}/{format_tokens(context_limit)} ({ctx_pct:.0f}%)"
                    )
                else:
                ...
                if output_limit > 0:
                    out_pct = (
                        (output_tokens / output_limit) * 100 if output_tokens > 0 else 0
                    )
                    parts.append(
                        f"OUT:{format_tokens(output_tokens)}/{format_tokens(output_limit)}"
                    )
                else:
    ...
    def log_request_error(
        ...
        log_tier: str = "default",
        traceback_str: Optional[str] = None,
    ) -> None:
        """
        Log request error, including the actual model that failed.

        Format (summary tier):
        🔴 abc123 | minimax-m2.5 | ERROR | 0.5s | Rate limit exceeded

        Format (default tier):
        🔴 abc123 | claude-3.5→minimax @opencode_go | ERROR | 0.5s | Rate limit exceeded

        Format (debug tier):
        🔴 abc123 | claude-3.5→minimax @opencode_go (https://opencode.ai/zen/go/v1) | ERROR | 0.5s | Rate limit exceeded
        """
        # Truncate error message if too long
        error_msg = str(error)
        if log_tier == "debug":
            max_len = 200
        elif log_tier == "default":
            max_len = 120
        else:
            max_len = 80
        ...
        if USE_RICH and console:
            ...
            if original_model and model_name and original_model != model_name:
                error_text.append(f"{original_model}→{model_name} ", style="cyan")
            elif model_name:
                error_text.append(f"{model_name} ", style="cyan")

            if provider:
                error_text.append(f"@{provider} ", style="yellow")

            if endpoint and log_tier == "debug":
                error_text.append(f"({endpoint}) ", style="dim")

            error_text.append("| ERROR | ", style="dim")
            error_text.append(error_msg, style="red")

            # Full traceback in debug mode
            if log_tier == "debug" and traceback_str:
                error_text.append("\n", style="dim")
                error_text.append(traceback_str, style="dim red")

            console.print(error_text)
        else:
            error_info = f"  ✗ {request_id[:8]} | ERROR"
            if original_model and model_name and original_model != model_name:
                error_info += f" | {original_model}→{model_name}"
            elif model_name:
                error_info += f" | {model_name}"

            if provider:
                error_info += f" @{provider}"

            if endpoint and log_tier == "debug":
                error_info += f" ({endpoint})"
