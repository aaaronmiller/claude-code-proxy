...
def check_vibeproxy_health(force_refresh: bool = False) -> Tuple[bool, Optional[str]]:
    ...
    try:
        response = httpx.get(
            f"{VIBEPROXY_BASE_URL}/health", timeout=VIBEPROXY_HEALTH_TIMEOUT
        )
        ...
        if is_available:
            ...
            logger.warning(
                f"VibeProxy health check failed: status {response.status_code}"
            )
...
class AntigravityAuth:
    ...
    def get_auth_data(self, force_refresh: bool = False) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if row:
                ...
                logger.debug(
                    f"[AntigravityAuth] Successfully loaded auth data from database"
                )
                return self._auth_data
            else:
                print(
                    f"ERROR [AntigravityAuth]: No row found for key '{self.AUTH_KEY}'"
                )
        except Exception as e:
            print(
                f"ERROR [AntigravityAuth]: Database access failed - {type(e).__name__}: {str(e)}"
            )
            pass
    ...
    def get_token(self, force_refresh: bool = False) -> Optional[str]:
        ...
        if self._cached_token and not force_refresh:
            logger.debug(
                f"[AntigravityAuth] Using CACHED token (first 8 chars): {self._cached_token[:8]}..."
            )
            return self._cached_token

        logger.debug(
            f"[AntigravityAuth] Fetching FRESH token from database (force_refresh={force_refresh})"
        )
        ...
        if auth_data:
            ...
            if new_token:
                ...
                if self._cached_token and self._cached_token != new_token:
                    print(
                        f"WARNING [AntigravityAuth]: Token CHANGED! Old: {self._cached_token[:8]}... New: {new_token[:8]}..."
                    )
                else:
                    logger.debug(
                        f"[AntigravityAuth] Token retrieved successfully (first 8 chars): {new_token[:8]}..."
                    )
                ...
                print(
                    f"ERROR [AntigravityAuth]: Auth data found but apiKey is missing!"
                )
        else:
            print(
                f"ERROR [AntigravityAuth]: No auth data found in database at {self.DB_PATH}"
            )
