...
class ActivityFeed(BaseModule):
    ...
    def render_dense(self) -> str:
        ...
        for req_id, pair in list(request_pairs.items())[-4:]:  # Show last 4 requests
            ...
            # Model routing
            original_model = self._format_model_name(
                start_req.get("original_model", "unknown")
            )
            routed_model = self._format_model_name(
                start_req.get("routed_model", "unknown")
            )
            ...
            if complete_req:
                ...
                if thinking_tokens > 0:
                    text.append(
                        f"| 🧠{self.format_tokens(thinking_tokens)} ", style="magenta"
                    )
                ...
                cost = self._estimate_cost(
                    input_tokens, output_tokens, thinking_tokens, routed_model
                )
                ...
                if duration_ms > 0:
                    text.append(
                        f" | ⚡{self.format_duration(duration_ms)}", style="dim"
                    )
    ...
    def render_sparse(self) -> str:
        ...
        for req in recent_requests:
            ...
            if req_type == "complete":
                ...
                has_completion = any(
                    r.get("request_id") == req_id
                    and r.get("type") in ["complete", "error"]
        ...
        completed_reqs = [
            req for req in recent_requests if req.get("type") == "complete"
        ]
        avg_duration = 0
        if completed_reqs:
            total_duration = sum(
                req.get("duration_ms", 0) for req in completed_reqs[-5:]
            )
        ...
        avg_str = (
            f"{self.format_duration(avg_duration)} avg"
            if avg_duration > 0
            else "no data"
        )
