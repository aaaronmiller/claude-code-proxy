...
class AnalyticsPanel(BaseModule):
    ...
    def render_dense(self) -> str:
        ...
        text.append(
            f" | Avg Response: {self.format_duration(avg_duration)}", style="green"
        )
        text.append(
        ...
        if durations:
            text.append(
                f"🏆 Fastest: {self.format_duration(fastest_duration)}", style="green"
            )
            text.append(
                f" | 🐌 Slowest: {self.format_duration(slowest_duration)}", style="red"
            )
        text.append("\n")
