...
from src.dashboard.model_display_utils import format_model_name
...
class PerformanceMonitor(BaseModule):
    ...
    def add_request_data(self, request_data: Dict[str, Any]):
        ...
        if request_data.get("type") == "start":
            ...
            if self.current_request and self.current_request.get(
                "request_id"
    ...
    def render_dense(self) -> str:
        ...
        for req in recent_requests:
            if req.get("type") == "start" and req.get(
                "request_id"
            ) == latest_complete.get("request_id"):
                start_req = req
                break
        ...
        if start_req:
            ...
            text.append(
                f" | {self._format_model_name(original_model)}→{self._format_model_name(routed_model)}",
                style="yellow",
            )
        ...
        if start_req:
            ...
            if context_limit > 0 and input_tokens > 0:
                ...
                text.append(
                    f" | 📊 CTX: {ctx_bar} {self.format_tokens(input_tokens)}/{self.format_tokens(context_limit)} ({ctx_pct:.0f}%)",
                    style="cyan",
                )
        ...
        if thinking_tokens > 0:
            ...
            if start_req and start_req.get("output_limit", 0) > 0:
                think_bar = self.create_progress_bar(
                    thinking_tokens, start_req["output_limit"], 10
                )
                text.append(
                    f"{think_bar} {self.format_tokens(thinking_tokens)} tokens",
                    style="bright_magenta",
                )
            else:
                text.append(
                    f"{self.format_tokens(thinking_tokens)} tokens",
                    style="bright_magenta",
                )

        # Cost estimate (rough)
        cost = self._estimate_cost(
            input_tokens, output_tokens, thinking_tokens, model_name
        )
        ...
        if start_req:
            ...
            if output_limit > 0:
                ...
                text.append(
                    f"📤 OUT: {out_bar} {self.format_tokens(output_tokens)}/{self.format_tokens(output_limit)}",
                    style="blue",
                )
    ...
    def render_sparse(self) -> str:
        ...
        for start_req in reversed(self.request_history):
            if start_req.get("type") == "start" and start_req.get(
                "request_id"
            ) == req.get("request_id"):
                context_limit = start_req.get("context_limit", 0)
                if context_limit > 0:
        ...
        parts = [
            ...
            f"📊{ctx_pct:.0f}%"
            if ctx_pct > 0
            else f"📊{self.format_tokens(input_tokens)}",
