...
class RoutingVisualizer(BaseModule):
    ...
    def render_dense(self) -> str:
        ...
        for req in reversed(recent):
            if req.get("type") == "complete" and req.get("request_id") == start_req.get(
                "request_id"
            ):
                complete_req = req
                break
        ...
        if complete_req:
            ...
            text.append(
                f"                  ↓ {self.format_tokens(output_tokens)} out",
                style="blue",
            )
            text.append("\n")
            ...
            if thinking_tokens > 0:
                text.append(
                    f"🧠 Thinking: {self.format_tokens(thinking_tokens)} tokens",
                    style="magenta",
                )
            ...
            cost = self._estimate_cost(
                input_tokens_actual, output_tokens, thinking_tokens, routed
            )
            ...
            if context_limit > 0 and input_tokens_actual > 0:
                ...
                text.append(
                    f"            📊 Efficiency: {efficiency:.0f}%", style="cyan"
                )
    ...
    def render_sparse(self) -> str:
        ...
        if start_req:
            for req in reversed(recent):
                if req.get("type") == "complete" and req.get(
                    "request_id"
                ) == start_req.get("request_id"):
                    complete_req = req
                    break
