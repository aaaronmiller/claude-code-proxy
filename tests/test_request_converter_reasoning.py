...
from src.services.conversion.request_converter import convert_claude_to_openai, _apply_reasoning_config
...
class TestRequestConverterWithReasoning:
    ...
    def test_convert_with_reasoning_suffix(self):
        ...
        # Disable custom prompts to avoid file path issues in tests
        model_manager.config.enable_custom_big_prompt = False
        model_manager.config.enable_custom_middle_prompt = False
        model_manager.config.enable_custom_small_prompt = False
        ...
        model_manager.is_newer_openai_model.return_value = True
    ...
    def test_convert_without_reasoning(self):
        ...
        model_manager.is_newer_openai_model.return_value = False
    ...
    def test_convert_with_anthropic_thinking(self):
        """Test conversion with Anthropic thinking config."""
        claude_request = ClaudeMessagesRequest(
        ...
        # Mock parse_and_map_model to return Anthropic thinking config
        reasoning_config = AnthropicThinkingConfig(type="enabled", budget=4096)
        model_manager.parse_and_map_model.return_value = ("gpt-4o", reasoning_config)
        model_manager.is_newer_openai_model.return_value = False
